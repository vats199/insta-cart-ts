...
router.post('/verifyOTP',body('phone_number').isMobilePhone('any')
                        .withMessage('Please enter a valid phone number!')
                        ,
                        body('otpValue').isLength({min: 4, max: 4})
                        .withMessage('Enter Valid OTP!')
                        , jwtAuth.jwtAuth, authController.verifyOTP ) ;                         

router.post('/otpLogin',body('phone_number').isMobilePhone('any')
                        .withMessage('Please enter a valid phone number!')
                        ,
                        body('otpValue').isLength({min: 4, max: 4})
                        .withMessage('Enter Valid OTP!')
...
router.post('/resetPasswordLink', body('email').isEmail()
                                    .withMessage('Please enter a valid email address!')
                                    .normalizeEmail(), authController.resetPasswordLink);

router.get('/resetPassword/:token', authController.getNewPassword);

router.post('/new-password',body('password', 'Please Enter a valid Password!').isLength({ min: 5 })
